...
и женщины, истрепанные, как пословица.""".trimIndent(), """Меня сейчас узнать не могли бы:
жилистая громадина
стонет,
корчится.
...
когда
я
абсолютно спокоен?""".trimIndent()))
        assertEquals("", longestCommonSubstring("", "")) // empty strings
        assertEquals("", longestCommonSubstring("NbA", "nBa")) // case-sensitivity
        assertEquals("Hell", longestCommonSubstring("HellRaisers", "RaisHellers"))
    }
    ...
    fun calcPrimesNumber(calcPrimesNumber: (Int) -> Int) {
        ...
        assertEquals(0, calcPrimesNumber(-345))
